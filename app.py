...
def parse_can_log(file_content, max_lines):
    messages = []
    ...
    for line in lines[:max_lines]:
        ...
        if (
            not line
            or line.startswith("***")
            or line.lower().startswith("date")
            or line.lower().startswith("base")
            or line.lower().startswith("begin")
            or line.lower().startswith("end")
            or "ErrorFrame" in line
            or "CANFD" in line
        ):
            continue
...
if dbc_file and log_file:
    ...
    try:
        ...
        messages = parse_can_log(log_content, int(max_lines))
        ...
        for index, msg in enumerate(messages):
            ...
            if total_messages > 0 and index % 1000 == 0:
        ...
        if decoded_rows:
            ...
            col1.metric("Parsed Messages", total_messages)
            ...
            st.warning(
                "No messages decoded. Try increasing maximum lines or check DBC/log compatibility."
            )
