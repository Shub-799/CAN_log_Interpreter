    ...
    for line in lines:
        ...
        try:
            ...
            if len(parts) >= 7 and parts[1] in ["Rx", "Tx"]:
                ...
                data_bytes = bytes(
                    int(x, 16) for x in parts[6:6 + dlc]
                )
            ...
            # 0.123456 1 109 Rx d 8 59 02 74 02 DE 3E 00 00
                ...
                can_id_text = parts[2]
                can_id_text = can_id_text.replace("x", "")
                can_id_text = can_id_text.replace("X", "")
...
if dbc_file and log_file:
    ...
    try:
        ...
        if decoded_rows:
            ...
            st.warning(
                "No messages decoded. Possible reasons: CAN ID mismatch, "
                "DBC mismatch, unsupported log format, or extended/standard ID mismatch."
            )
