...
st.write("Supports BUSMASTER .log and DBC based CAN signal decoding.")

dbc_file = st.file_uploader("Upload DBC File", type=["dbc"])
log_file = st.file_uploader("Upload CAN Log File", type=["log", "txt"])

def parse_busmaster_log(file_content):
    messages = []
...
if dbc_file and log_file:
    ...
    with open("temp_uploaded.dbc", "wb") as f:
        f.write(dbc_file.getbuffer())

    try:
        db = cantools.database.load_file("temp_uploaded.dbc")

        log_content = log_file.getvalue()
        messages = parse_busmaster_log(log_content)
        ...
        if decoded_rows:
            ...
            st.warning("No messages decoded. Possible reasons: CAN IDs mismatch, DBC not matching log, or extended/standard ID mismatch.")

    except Exception as e:
        st.error(f"Error: {e}")

else:
    st.info("Please upload both DBC and BUSMASTER log file.")